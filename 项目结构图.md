...
flowchart LR
    ...
    C -->|是| F{"综合报告<br/>是否存在?"}
    F -->|否| G["创建7个并行分析任务"]
    F -->|是| H["跳过分析<br/>直接生成社工方案"]
    E --> G
    G --> I["并行执行AI分析"]
    I --> J["生成综合报告"]
    J --> L["生成社工攻击方案"]
    L --> K["保存Markdown报告<br/>+ 社工方案"]
...
- **新增**：如果综合报告已存在，跳过7个并行分析任务，直接生成社工攻击方案（灰色高亮）
- AI分析采用7个任务并行执行，提高效率
- 最终生成Markdown格式的分析报告
- **新增**：基于综合报告和CSV数据，生成社会工程学攻击方案（红色高亮显示）
...
如果文件夹中已有综合报告（`{base_filename}_report.md`）和CSV文件，程序会**自动跳过**信息收集和7个并行分析任务，直接读取已有的综合报告生成社工攻击方案，避免重复消耗API tokens。
